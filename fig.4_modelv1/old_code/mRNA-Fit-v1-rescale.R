...
fun.scale <- function(x) (x-min(x))/(max(x)-min(x))
...
all.equal(rownames(genes.kdeg),rownames(genes.normCount))

#genes.normCount <- genes.normCount/rowMax(genes.normCount[,1:14]) #normnizsed by wt
rescaled.date <- cbind(t(apply(genes.normCount[,1:14], 1, fun.scale)),
                       t(apply(genes.normCount[,1:14], 1, fun.scale)))
load(file="../fig.1_wt/kcluster_b1.Rdata")
require(pheatmap)
pheatmap(tmp[k_ord$gene,],scale = 'none',cluster_rows = F,cluster_cols = F)
