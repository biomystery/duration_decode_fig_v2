...
runModel.v1.2<- function (nfkb_input,pars,times=manytp){
  ...
  k0 <- pars$k0;kt<-pars$kt;k_deg<-pars$k_deg;kd <- pars$kd;
    mRNAini <- c(mRNA= (kt*(nfkb_input$val[1])^n/(kd+(nfkb_input$val[1]))^n+k0)/k_deg) # init,ss assumption
  
  ## subroutine: The model
  odeModel <- function (Time, State, Pars) {
    with(as.list(c(State, Pars)),{
      nfkb <-approxfun(nfkb_input$time,nfkb_input$val)(Time)
      dmRNA    <- kt*nfkb^n/(kd+nfkb^n)+k0-k_deg*mRNA
      return(list(c(dmRNA)))
    })
  }
...
v1.fit.2 <- mclapply(rownames(genes.kdeg),
                   FUN = function(x) runOptim.v1.2(initP = initP,lb = lb,ub = ub,
                                                 g.name = x,input.nfkb.ctl = input.emsa.ifnar,
...
save(file = 'v1.fit.2.n2.RData',v1.fit.2)
