...
# load parameter  ---------------------------------------------------------
head(v1e.res)
all(v1e.res$X %in% gene.dic$gene)
...
dic.v1 <- cbind(dic.v1,v1e.res[,2:8])
write.csv(file="table_model.csv",cbind(dic.v1,v1e.res[,2:8]))

ev$pd %>% filter(type=="Sim.") %>% group_by(gene)%>% 
...
nrmsd.v2 <- read.csv(file = './models/mRNA-Fit-avg-sp-v1f/result.csv',
                     stringsAsFactors = F)
