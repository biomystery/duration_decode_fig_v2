...
range(plot_data2[,1:21])
range(plot_data)
range(plot_data2)
colnames(plot_data2)
all.equal(plot_data2,plot_data)

#write.csv(file='./maxscale.csv',plot_data2)
...
write.csv(file="sp.fraction.maxscale.csv",deltaAuc.all)
...
#cols[4]<- cols[5]
cols[3]<- cols[4] <- "azure"
pheatmap.2(pd=log2(deltaAuc),bks=bks,cols=cols,legend=T)  
sapply(1:4,
       function(i){
         setEPS()
         postscript(paste0(fig_folder,"dAUC_maxFrac_genoScale",i,".eps"),
                    onefile = F,width = 6,height = 6)
         pheatmap.2(pd=log2(deltaAuc[,i]),bks=bks,cols=cols,legend=F)  
         dev.off()
       })
