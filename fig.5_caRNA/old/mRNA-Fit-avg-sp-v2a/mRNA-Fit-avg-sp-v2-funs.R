...
runModel.v1<- function (nfkb_input,k_deg_input,pars,times=manytp){
  k0 <- pars$k0;kt<-pars$kt;kd <- pars$kd;
  mRNAini <- c(mRNA= (kt*nfkb_input$val[1]^N/(kd^N+nfkb_input$val[1]^N)+k0)/k_deg_input$val[1]) # init,ss assumption
...
runOptim.v1 <- function(initP,lb,ub,g.name,input.nfkb,input.nfkb.mt){ 
  ...
  obj <- function(pars){
    ...
    out <- sapply(c("LPS","TNF"), function(x)
      runModel.v1(nfkb_input = data.frame(time= input.nfkb$Time,val = input.nfkb[,x]),
                  k_deg_input= data.frame(time=genes.kdeg.times,
                                          val=eval(expr = parse(text = paste0("as.numeric(genes.kdeg.",tolower(x),"[g.name,])")))),
                  pars= data.frame(k0=pars[1],kt=pars[2],kd=pars[3]),
  ...
  out  <- sapply(c("LPS","TNF"), function(x)
    runModel.v1(nfkb_input = data.frame(time= input.nfkb$Time,val = input.nfkb[,x]),
                k_deg_input= data.frame(time=genes.kdeg.times,
                                       val=eval(expr = parse(text = paste0("as.numeric(genes.kdeg.",tolower(x),"[g.name,])")))),
                pars= data.frame(k0=fit$par[1],kt=fit$par[2],kd=fit$par[3]),
                times = times),simplify = F)
  ...
  fit$simScale.mt <- max(unlist(lapply(out.mt, function(x) subset(x,subset =out$LPS$time %in% mRNAprofile$time)$mRNA)))
  fit$gene <- g.name
  return(fit)
}
