...
# v1 plot 
ev.1 <- ev
rownames(nrmsd.v12) <- nrmsd.v12$gene; 

# added TS modl
nrmsd.ts <-read.csv(file = '../fig.4_modelv1/v1f_v1de_cmp.csv',
                    stringsAsFactors = F)
nrmsd.v12$gene <- rownames(nrmsd.v12)

nrmsd.v12 <- nrmsd.v12 %>% left_join(nrmsd.ts[,-c(1,3)],by = 'gene')
names(nrmsd.v12)[5] <- 'nrmsd_v1a'
nrmsd.v12 <- nrmsd.v12[,c(4,1,5,2,3)]

if(T){
  #p.genes <- c('Fpr1','Lcn2','Tnfsf11','Ccl1')
  p.genes <- unique(ev.1$pd$gene[!ev.1$pd$v1 & ev.1$pd$clust==7])
  ...
  p.v1<- plotEgFit(eg.genes = p.genes,simData = v1.simData,
    ...
    geom_text(data=cbind(nrmsd.v12[p.genes,1:3],type='Sim.',sti='lps'),
              aes(x=4,y=1.1,label=signif(nrmsd_v1,2)))
  
  # v1 data 
  v2.simData <- read.csv(file='../fig.4_modelv1/mRNA-Fit-avg-sp-v1f/bestFit.tc.csv',
                         stringsAsFactors = F)
  ...
  p.2 <- p.2 + geom_text(data=cbind(nrmsd.v12[p.genes,1:3],type='v2_Sim.',sti='lps'),
              aes(x=4,y=1.1,label=signif(nrmsd_v1a,2)))
  print(p.2 )
  
  # print 
  print(nrmsd.v12[p.genes,1:3],digits = 2)
}
p.3
...
write.csv(file='../data/caRNA-exon-rpkm.csv',reads.exon.rpkm)
