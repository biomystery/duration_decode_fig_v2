...
fig_dir <- "~/Dropbox/Projects/DurationDecoding/figure/Fig.6/subfigs/"
...
require(gridExtra)
ggsave(filename = paste0(fig_dir,'subfig6B_egs.eps'),
...
fig5.setting <- read_rds(path = "../fig.5_caRNA/pd.fig5.Rds")
...
plotSimHm <- function(p_species="caRNA"){
  ...
  for(i in 1:4){
    ...
    postscript(paste0(fig_dir,"subfig6c_sim",p_species,i,".eps"),onefile = F,
               width = wid,height = 10)
...
plotLegend(cols="Reds",
           bks=seq(0,1,by = 0.1),
           fnames = paste0(fig_dir,"subfig6c_simcytoRNA_lg.eps"))
plotLegend(cols="Blues",
           bks=seq(0,1,by = 0.1),
           fnames = paste0(fig_dir,"subfig6c_simcaRNA_lg.eps"))

# scoreMatix 
v1 <- readRDS(file = "../fig.4_modelv1/fig4.setting.rds")$hm_data[,1:2]
...
setEPS()
postscript(paste0(fig_dir,"subfig6c_sim_score_v2.eps"),
...
setEPS()
postscript(paste0(fig_dir,"subfig6c_sim_score_v1.eps"),
           onefile = F,width = 1,height = 10)
