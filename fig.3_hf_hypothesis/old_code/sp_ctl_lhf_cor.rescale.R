...
genes.sp <- read.csv(file="../fig.1_wt/sp.log2fraction.rescaled.csv",row.names = 1,stringsAsFactors = F)
...
pd$lt_sp[pd$lt_sp < -4] <- -4 
pd.cor <- cor.test(x = pd$hf_log10,y = (pd$lt_sp))

pdf(file='sp_ctl_lhf_cor.rescale.pdf')
par(mfrow=c(2,2))
...
pd.sp.ccl5 <- log2(sum(construct.raw$ccl5.tnfp/max(pd.tmp))/sum(construct.raw$ccl5.lpsp/max(pd.tmp)))
pd.tmp <- c(construct.raw[,5],construct.raw[,6])
pd.sp.fos <- log2(sum(construct.raw$fos.tnfp)/ sum(construct.raw$fos.lpsp))

matplot(construct.raw$Time,construct.raw[,2:3]/max(construct.raw[,2:3]),type = "b",
        pch=1,lty=1,ylab = "frac. of max",xlab="Time (hr)", ylim=c(0,1),
        main=paste0("Ccl5,TL_sp=",signif(pd.sp.ccl5,2),' log2'))
legend(4,.4,legend = c("TNFp","LPSp"),lty=1,col=c(2,1))

matplot(construct.raw$Time,(construct.raw[,5:6])/max((construct.raw[,5:6])),type = "b",
        pch=1,lty=1,ylab = "frac. of max",xlab="Time (hr)",
        main=paste0("Fos,TL_sp=",signif(pd.sp.fos,2),' log2'))
par(mfrow=c(1,1))
